...
## DORA duty
Documentation is doc-only work: it fires no item state event and writes no ledger.
State changes (for the items you document) are recorded by their owning agents via
`make wi-append`; metrics are DERIVED by `make wi-project`; the DORA CSV ledger is
FROZEN — do not write it.
...
defects/, design/, dora/, fixtures/, infra/, items/ (with items/active/ and items/done/),
observability/, scripts/, secrets/, slices/, spike/, and the DERIVED views/ folder — note in
its README that queues/state/tree/stats are generated by `make wi-project`, are never
hand-edited, and that the old `queues/` folder is superseded). Each README must:
