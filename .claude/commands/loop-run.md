...
> events below (`made_ready`/`pulled`/`built_green`/`validated`/`build_failed`/`rejected`/
> `blocked`/`unblocked`) ARE those appends; there is no `dora record`, no CSV edit. Read
...
4. **Run the inner dev loop for each pulled UC, concurrently** (isolated by §40
   ...
   - **Blocked-reason (§F7a):** whenever a UC is blocked (gate hold, collision stop, rework),
     ...
     `EVENT=unblocked`. The reason rides on the event, so the board banner is derived — no
     separate `blocks.csv` to keep in step.
