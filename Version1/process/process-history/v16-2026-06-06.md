...
## 35. Validation-as-code with run provenance (v16 — human-directed)

**The tester does not improvise validation through ad-hoc bash** (generated
test data, one-off `curl` probes, CLI spot-checks). Those commands block on
prompts, are thrown away after use, and leave no record of what was verified,
when, or against which build. Instead:

1. **Every validation check is committed code in the project's validation
   framework** (`tests/validation/` beside `tests/smoke/`), runnable through
   the allowlisted runners (`npx playwright test`, `npx vitest run`) — which
   is also what makes it prompt-free (§33). Test *data* (planted attacker
   values, fixture bodies) lives in the spec, not in a shell one-liner.
2. **Specs are lifecycle-managed by relevancy.** Each spec file header states
   the slice and acceptance cases it pins, and its relevancy: `pinned`
   (regression — runs every validation pass) or `point-in-time` (verified a
   one-off property — retired once superseded). At slice-next and retro the
   tester reviews the set: add what the new slice needs, **delete** what no
   longer earns its run time. Removal is a normal, logged act — the spec
   remains in git history with its run records.
3. **Every run is recorded against the iteration.** Each validation run emits
   a `validation_run` ledger row: project, iteration, slice, suite, the
   **sha under test**, and pass/fail. "Was S1 ever verified, and against which
   build?" must be answerable from the ledger, not from a conversation
   transcript.
4. **CLI-only checks** (IAM policy contents, reserved concurrency, cache
   policy ids) are wrapped the same way: a committed spec that shells out via
   the allowlisted read-only AWS patterns and asserts on the output — not
   pasted interactively.

**Targets:** lead time (tester prompt-pauses → 0) and CFR/MTTR (validation is
re-runnable: any regression can be re-checked in one command, and recovery
evidence is a recorded run, not a claim). **Anticipated effect:** tester step
runs entirely through committed, allowlisted entry points; full audit trail of
which ACs were verified at which iteration against which sha.
