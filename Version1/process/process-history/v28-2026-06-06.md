...
# Retro scoring at retirement (s005-h1 delivery retro, 2026-06-06)

DORA: lead=2979s freq=6/day (doubled) cfr=26% mttr=4202s constraint=tester.

| Change | Observed (iteration 7) | Verdict |
|---|---|---|
| v25 §30 walking-skeleton probe | First outing: surfaced rate-rule timing semantics + the masked-200 anomaly that predicted DEFECT-WAF-001; attachment+transparency proven pre-validation | MET |
| v25 code↔policy pin | No AccessDenied-class failures this slice | HOLDING |
| v28 local-standability gap list | Applied consciously (WAF cannot stand locally → controls mapped); recorded in route header | MET |
| Region policy | First exception exercised: platform-forced us-east-1, documented, minimal stack, §30-pinned | MET |
| §37 parallel build (2nd outing) | UC1∥UC2 zero collisions; UC2's unbuildability did not waste UC1's lane | MET |
| §38 selection | Full first cycle: risk item → planned in prior slice's validation window (§8b) → delivered | MET |
| v23 self-service | Tester built waf-sustained target + fixed own spec bugs; engineer built waf-probe + Makefile target | MET |
| §40 UC flags | Still unexercised (file ownership sufficed again) | CARRY — consider scoring moot next retro |
| Platform-semantics class | Strikes 2+3 cost ~10min + same-phase discovery (vs 97min at strike 1); skill + synth contracts now pin WAF associability AND block-code/CF-error disjointness | CLASS COST COLLAPSING |
