...
supersedes: v29
status: superseded by v31 (2026-06-07, focused retro: shared change-impact model)
---

# Retirement note — anticipated-vs-observed scoring (done at the v31 retro)

| Change | Anticipated | Observed | Score |
|--------|-------------|----------|-------|
| v30 §5a failure semantics (5xx=callee/4xx=caller) | defects classified + routed without debate; CFR/MTTR attribution honest | Tester classified DEFECT-H2-003 as "5xx from a service WE own → engineering defect" same-day, correctly, unprompted | MET (early, 1 point — keep scoring) |
| v29 §19 trunk-CD prereq corollary | no enforcement-before-prereq pushes | **MISSED**: DEFECT-H2-001 (s005-h2 A1 mint pushed before A2 secret existed; ~35 min prod outage) was exactly this class. Healed by re-asserting the schedule (f000aea), not by compensation — the *recovery* honoured §19 even though the *planning* didn't. Corollary text stands; the gap was route-construction not reading it (v31 dependency model read-before-build is the structural fix) | PARTIAL |
| v29 WAF evaluation-semantics skill note | no repeat WAF-timing false-reds | No WAF work this iteration | N/A carry |
| §40 use-case flags (carried unscored since v21) | parallel WIP isolated by flags, no stash choreography | **SCORED THIS ITERATION**: H2_ENFORCE executed the full §40 lifecycle on a real shared seam (introduce OFF b79ffbc → Set B ships credentials → flip default-ON 4ef7319 → factor-out pending validation). Two-phase rollout prevented enforcement-before-credentials prod breakage by design | MET |

DORA at retirement: lead=3618s freq=6/day cfr=35% mttr=1794s constraint=tester.
CFR regression (27→35%) driven by 3 in-slice prod defects (H2-001/-002/-003): one §39
schedule violation, two platform-semantic class (identitySource AND = strike 5;
lazy-TTL stale counter — mocked-adapter tests cannot see platform semantics).
MTTR halved (3054→1794s median) — walking-skeleton probe + same-session recovery
discipline working.
