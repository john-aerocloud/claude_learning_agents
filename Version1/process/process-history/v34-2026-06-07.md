...
supersedes: v33
status: superseded by v35 (2026-06-07, s007 retro: gate-4 timing under trunk-CD)
---

# Retirement note (scored at the s007/v35 retro)
| Change | Anticipated | Observed | Score |
|---|---|---|---|
| v34 §25a integration step (EXP-011) | integrated agents perform >= as well, shorter defs | Pass 1 (6 experiments): all 47 mechanisms intact; s007 ran ENTIRELY on integrated defs — read-before-build STOP, §19 ordering, probes, §5a stop, budget hygiene all fired; 2 more integrations at this retro (EXP-009/010) = 3-integration horizon reached | ON TRACK (1 caveat: net line reduction marginal) |
| v33 §11a use-case flow (EXP-010) | defects at UC probes not slice validation | s007: disconnect skeleton (UC1+UC3 §11a done condition) caught the guest-survivor render gap BEFORE the tester; per-UC deploys rode concurrency groups cleanly | VALIDATED 2/2 |

DORA at retirement: lead=4325s freq=8/day cfr=26% mttr=2033s constraint=tester.
...
2. **validated** — anticipated effect observed at retro scoring. The change is
   ...
   git, not in the agent's prompt. Integration is a SIMPLIFICATION pass:
   the agent file must end shorter or equal, never longer, and the behaviour
   must survive intact (the next retro spot-checks the mechanism still
   fires). After integration the row drops out of active tracking (spot
