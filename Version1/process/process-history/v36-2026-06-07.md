...
supersedes: v35
status: superseded by v37 (2026-06-08, IMP-007 surfaced node-id/@covers canonical-form drift)
---

# Retirement note (scored at IMP-007 build)
v36 ran clean through s005-h3 (4th 0-defect slice). v37 adds ONE §12a sentence:
node ids and @covers tags share ONE canonical form (kebab-case) — IMP-007's
real-data run surfaced camelCase ids (domainMove) vs kebab tags (@covers
domain-move) reporting as false-uncovered; the tool correctly refused to
fuzzy-match (that would hide the drift the model exists to expose).
