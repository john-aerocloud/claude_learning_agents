...
Status: **design proposal** (not yet an accepted process change). Written to debug a
recurring drift defect and to evaluate whether the multi-queue model should be replaced
by a single self-contained-work-item model.
...
4. **Migration is itself risky.** Rewriting the work-tracking substrate mid-project is how
   you *create* drift. OagEventSource is complete — do **not** big-bang it. Start the new
   model on the **next** project (or a shadow model run in parallel on one small project),
   prove it, then it becomes the default. OagEventSource stays as-is, archived.
...
Adopt the model, phased, as a scored process experiment:

1. **Write the type graphs** (use-case, defect, chunk, slice, JTBD) as declarative data:
   states, per-state agents, transitions, event names. This is the auditable core.
2. **Build one reducer** — `fold(item.events, type_graph) -> state` — with edge-checking on
   append (reject illegal transitions; an illegal transition surfaces an
   amendment-proposal). Tested once, called everywhere.
3. **Make every other view a pure projection** of the item set: queues, board status,
   DORA, dependency tree. Delete `state.md`, `queues/*.csv`, and the `items.csv` state
   columns as *stored* state — regenerate them as read-only views if humans still want the
   files.
4. **Store edges one-directional; derive the reverse.**
5. **Pilot on the next project**, not OagEventSource. Score against the current baseline:
   drift incidents (target 0), reconcile token-cost per resume (target ↓), and pull-cycle
   latency (watch it doesn't regress).
