...
| EXP-005 | v31 (2026-06-07) | §12a + engineer/tester/architect/product defs | shared change-impact model (mermaid deps, updated-in-commit, read-before-build/test, @covers) | tester (constraint), CFR, MTTR | tester median < 900s; no defect in a changed-but-untested area; 0 §19-class misses | 2 | **validated** (2/2) | (1) read-before-build stop prevented a DEFECT-H2-001-class prod AccessDenied pre-code; (2) tester model-diff planning ~15min vs ~45-60min archaeology at s005-h2 (tester's own honest assessment); s006 = 0 prod defects. Sharper re-score at s007 against a settled baseline model |
...
| EXP-009 | s005-h2 retro (2026-06-07) | tester.md | budget-aware validation: when the system under test rate-limits by source, connection-consuming specs run serialised/ordered so the SUITE does not exhaust the budget it is validating | tester (false-red re-runs), CFR honesty | 0 test-isolation false-reds on rate-limited surfaces | 2 | active (1/2 MET) | s006: tester recorded budget state at start, serialised WS specs, waited out windows — 0 false-red defect claims; the remaining full-suite exhaustion is an INFRA gap (OI-34 WAF CI-IP exception), not tester behaviour |
| EXP-010 | v33 (2026-06-07) | §11a + engineer.md | use-case flow: deployable UC done = deployed + engineer probe green in prod; deploy order by concurrency group + §19 edges; builds overlap per §37 | lead time, deploy freq, MTTR attribution | defects surface at UC probes not slice validation; tester load shrinks (constraint protected); no deploy overwrites | 2 | active (1/2 MET) | s006: move-skeleton was UC3's done condition (3/3 vs prod incl. forged-gameId before flag flip); UC4 deployed dark per-commit; 0 defects reached the tester — the §11a mechanism fired as designed |
