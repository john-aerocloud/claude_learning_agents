...
| EXP-119 | v121 (2026-07-30) | cicd.md (first-ever fresh-account deploy: grant the FIRST-USE auto-provisioned resources) + improvement-slice IMP-026 (executable preflight) | **Problem:** AdixOut's first-ever prod deploy failed TWICE on missing create-permissions for account-level bootstrap singletons AWS/SST/EventBridge lazily provision on FIRST USE — (1) `ecr:CreateRepository` on the SST `sst-asset` asset repo (deploy role, run 30537306744), (2) `iam:CreateServiceLinkedRole` for `AWSServiceRoleForAmazonEventBridgeApiDestinations` (onboarding EXEC role, DEF-AIDX-009). Both were INVISIBLE in dev because dev created those singletons long ago and stopped exercising the create path; dev-green + local synth both passed. Each miss = a failed prod deploy + a rework loop, and the SLR miss shipped as a prod-onboarding defect (DEF-AIDX-009) caught only by live prod validation. **Solution:** cicd.md — before any first-ever deploy into a fresh account, audit BOTH the deploy role AND every runtime exec role for the create permissions of the first-use resources the stack implies (SST/CDK asset store incl. the `sst-asset` ECR repo; a scoped `iam:CreateServiceLinkedRole` — ARN + `iam:AWSServiceName` condition, never `*` — for every AWS service the app uses that lazily creates an SLR); grant them in BOTH stages' policy source for fresh-account self-heal + dev↔prod parity. Executable preflight queued as IMP-026. | CFR on first-account deploys + deploy MTTR (a fresh-account first-use permission gap is caught before the deploy, not by a failed prod deploy + rework) | over the next 3 first-ever-into-an-account deploys (any project): the fresh-account grant-parity audit/preflight runs and 0 deploys fail on a missing create-permission for a first-use auto-provisioned resource (ECR asset repo, SLR, asset bucket); if a first-account deploy fails on such a missing grant, FAILED | 3 first-account deploys (or 3 retros) | active (0/3) | applies-to: any first-ever deploy into a fresh AWS account. Founding: AdixOut first prod stand-up (UC-AIDX-039) + DEF-AIDX-009, principle-failure `2026-07-30-adixout-first-prod-deploy-fresh-account-bootstrap-gaps.md`. Sibling of EXP-094 (policy verb-completeness) + EXP-096 (policy applied-per-stage) — this closes the THIRD gap: content complete + applied is still not enough when dev never exercised the fresh-account first-use create path. Opened at cap (8th active); EXP-101 is adopt-eligible next cycle to reopen a slot. |
