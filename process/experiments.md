...
**Opened** v142 (2026-08-20, ROC), gap-closing retro on `DEF-ROC-073`/`074`/`075`/`076`.
...
The active table holds **8 OagEventSource rows**, and ROC's own **EXP-136..142 have no table rows
at all** — they exist only as `##` sections in this file. So the standing "8 active, AT cap" reading
is partly an artifact of ROC's rows being invisible to the very registry that caps them. EXP-143 is
given a real row. The **per-instance allowance v141 flagged as owed is still owed**, and it is now
actively blocking honest accounting rather than merely being untidy. Flagged, not silently resolved:
ROC has no standing to adopt-or-kill another project's rows (v141's own ruling).
