...
<!-- v178 (ROC retro 2026-09-15, incident-triggered: 10 items since the last close — DEF-ROC-147,
158, 162, 163, 165, 168, 183, 184, 185 and UC-ROC-119 rework). Focus question: *what was the largest
contributor to gross lead time, and what reduces it while protecting DORA?* ANSWER: `reported` —
630.5 days, 35.4%, more than the next two states combined, and orchestrator-owned. Root cause at
level 5: recording a decision on a defect CONSUMES A WIP SLOT, because the only decision event from
`reported` is `triaged` and `triaged` lands in wip — so §F9b and loop-gate check 3 are in direct
mechanical opposition whenever wip is over cap, which it was (18/8). Routed: §F9i (a decision is
free; only starting work costs a slot) and §F9j (a dispatch is an event on the item — four agents
died to a 600s watchdog this cycle leaving items that LOOKED busy). EXP-ROC-020/021, both with a
negative arm that kills them. Reconcile latency ~0 (v177 authored and on main at 11:32), so §0a
Rule 4 is honoured and is not the constraint. -->
...
**OCCUPANCY IS NOT ACTIVITY. THE GUARD BUILT TO STOP THE LOOP STOPPING READ THE STALL AS HEALTH, AND LICENSED IT FOR SIXTEEN DAYS.**

**THE MEASUREMENT, which is the whole finding.** ROC's loop uptime since the last retro close is **10.0%** — 14.1 quiet days out of 15.7, in five separate quiet periods (5.1d, 4.2d, 3.0d, 23.2h, 20.5h). All-time it is **23.9%**: 50.2 of 66 days. That number did not exist before today; `make loop-uptime` computes it from timestamps the item logs already carried, so it could have been read at any point in the last three months and never was.

**AND THE LOOP DID NOT SIMPLY STOP — WHICH IS WORSE.** It ran on at least five separate days in that window and produced **128 events**, while `DEF-ROC-145`, `-147`, `-151`, `-154`, `-163`, `-165` and `UC-ROC-117` recorded **nothing at all**. Sessions happened and walked straight past the same eight stalled items every time. `DEF-ROC-153`'s fix sat pushed, green and unvalidated in `validating` for **15.8 days** with no tester ever dispatched, and its own validation note says so.

**WHY NOTHING CAUGHT IT.** `.claude/hooks/loop-continue.mjs` (§F13a, `EXP-ROC-016`) is the one mechanism that blocks a turn ending while the loop could still pull. Its capacity test was `wip >= cap` on **queue DEPTH**, with the comment *"waiting on agents at cap is legitimate — that is the loop working, not stalling."* ROC's `wip` has been **16 against a cap of 8 since 2026-08-29 with nine slots idle**, so that branch was true *because* work had stalled. Demonstrated on the identical state rather than argued: the pre-fix hook reports `nothing pullable (ready 7, wip 16/8, rework 0)` and **allows**; the fixed hook **blocks**, naming eight items idle up to 16.1 days. Widening the staleness window to 1000h reproduces the old allow exactly, which isolates idleness as the cause.

**THE FAILURE SHAPE, because it is not a typo.** The guard's stall signal and its healthy signal were **the same number**, so the worse the stall got, the more confidently it permitted the stop. This is §17i's family — *a control that cannot report is not a control, and silence is never a pass* — in its most expensive form yet: not a control that says nothing, but one that says **"all clear"** in precisely the circumstance it was built to catch. `loop-gate` had the right words all along (*"THIS DEPTH IS OCCUPANCY, NOT ACTIVITY … those slots are NOT capacity in use"*) and the hook simply never asked it. **Two controls in one system, one of which knew, and no path between them.**

**THE SAME SHAPE, THREE MORE TIMES THIS CYCLE — this is a family, not an incident.** (1) `DEF-ROC-068` sat `blocked` for **18 days** as *"not ours to patch"*; its probe correctly re-ran the real `npm audit` every cycle and correctly reported STILL BLOCKED every time, because a probe that re-observes the SYMPTOM can never test the PREMISE — and the premise was false, `flowbite-react` being our own direct dependency all along. **A park's probe must be able to falsify the reason for the park, not merely re-witness its effect.** (2) `DEF-ROC-153`: a third party moved a floating tag onto a broken commit, GitHub refused to load the action, and the gate job **died in 19 seconds** — a gate that dies reads the same as a gate that passes to anyone not reading the verdict line. (3) `DEF-ROC-166`: its `fixed` ref `29661dd` **is** an ancestor of `origin/main`, and so is `ce4d7e1`, its own revert twelve minutes later; `closedGuidanceText` appears nowhere in `src/dashboard` at HEAD. **Ancestry is not a done-check.** One revert in the last 200 commits, and it produced a false green — a 100% hit rate on the only occasion the class could fire.

**THE RULE, stated once for all four:** *a control may not read the symptom of the failure it guards against as evidence of health.* Wherever a check answers "is it fine?" by measuring a quantity that ALSO rises when things go wrong — depth, ancestry, the persistence of an advisory, a job that exits early — it must be re-expressed against the thing it actually cares about: activity, effect, premise, verdict.

**WHAT CHANGED.**
1. `.claude/hooks/loop-continue.mjs` — capacity is measured on **ACTIVITY**: a `wip` slot whose item has recorded no event for `LOOP_STALE_HOURS` (default 24, matching `loop-gate`) is not an agent working, it is **work owed a decision**, and it blocks the stop **in its own right regardless of `ready` depth**. The block names the idle items with ages and the three decisions (re-dispatch / release with a reason / append the event already earned), and says explicitly **do not raise the cap — the cap is not what is full**. Fails open and stays bounded at 3 consecutive blocks, both non-negotiable; when activity cannot be established it degrades to the old depth reading and **says so in the message** rather than claiming a reading it does not have.
2. `.claude/tools/loop-uptime.js` + `make loop-uptime` — **read it before quoting a constraint movement (§5b).** Downtime is real gross lead time and is never netted off, but it is not evidence about how any agent performed. Declared as a proxy, not an attendance record: a running session that produced nothing reads as quiet, and its bias is toward OVER-stating uptime. This mechanises the principle failure recorded on **2026-08-24** (`a-constraint-movement-quoted-without-loop-uptime`, 60.9h of downtime accounting for 72% of that constraint's median) and then left as prose. **A recorded root cause that is left un-mechanised does not stay the same size: 60.9 hours became 14.1 days.**
3. `process/machinery/state-graphs.json` — **every non-terminal state in every graph can now record that its subject went away.** `DEF-ROC-166` was trapped in `validating`: `validated` would be false and `rejected` would send it back to fix a state with no referent. The tester and the flow-manager each named it independently and each correctly refused to fire a false event — the system working — while the item held a `wip` slot against a breached cap, which is the system paying. Covered by a **sweep**, not a spot-check, and the sweep immediately found a second, unreported hole (`use-case.deploying`) that pinning the reported pair would have left standing. 636 machinery tests green.

4. `process/machinery/state-graphs.json` **v12** — **an open-item that carries real engineering work could not say so.** The `open-item` graph had exactly ONE progress edge (`open --scheduled--> scheduled`) and then went straight to `done`: no state meaning *someone is working on this right now*. So such an item contributed **nothing** to `time_in_state`, **nothing** to `by_owner` and nothing to any stage metric — the work teleported from `scheduled` to `done`. Found TWICE independently on the same day: `OI-ROC-013` claims it in prose (and had sat SCHEDULED, unpulled, for **16.2 days**, the oldest aging item in `ready`), and `make wi-append EVENT=pulled` was REFUSED on `OI-ROC-016`, a code fix to `design-quality-gate.mjs` with an engineer building it. **This retro named its constraint from `by_owner` — a table that structurally could not see either item.** Fixed by reusing the EXISTING `building` state (already owned by `engineer`, already mapped to `wip`), so the work becomes visible with no new vocabulary; the direct `scheduled --closed--> done` route is KEPT, because the common open-item is decision debt closed by a decision, and the two kinds now have two honest routes instead of one that lied about half of them.

**WHY IT HID FOR SO LONG, which is the transferable part:** the metric it silently zeroed is **one nobody expected to be non-zero**, so the zero read as a fact about the world rather than as a number that *could not come back non-zero*. That is the §E plumbing-cost-reads-0.0% shape, and it is the same family as the headline: a quantity that cannot report the bad case, mistaken for a quantity reporting good news. **So the audit question generalises past the missing `cancelled` exit to: of each type, can an item of this type be in a state that says someone is working on it?** Both questions are now committed as SWEEPS over every graph, not spot-checks of the reported instance — which is what found `use-case.deploying` and would have found `open-item` years earlier.

**TWO EXISTING GATES CAUGHT REAL CONSEQUENCES OF THIS CHANGE AND ARE WORTH NAMING, because they are the counter-example to the headline — controls that did exactly what they were built for.** (1) The v10 two-directional gate REFUSED the change while `metric_events` declared `cicd` a producer of `deploy_failed` and `open-item`'s owners excluded it — *an edge nothing can fire is not a fix*. Resolved by adding `cicd` to `open-item`'s `default_owners`, which **narrows a spoof rather than widening a permit**: §G already lists `OI-ROC-016 | open | cicd` as the single `spoof_indicator` in the last 20 items, i.e. cicd was already working open-items and had to borrow a name to say so. (2) The board-mapping gate refused it while `open-item.building` had no board status — unmapped, an in-progress item renders as unstarted Backlog. Both fixed in the same change; **640 machinery tests green.**

**WHY `cancelled` AND NOT A NEW `superseded` TERMINAL**, since the choice was offered and the reasoning is the useful part. The metric argument for a distinct terminal *looks* decisive and is not: `_compute_quality` counts **every** non-annotation exit from `validating` in the denominator while only `rejected` is a failure, so a cancellation dilutes the tester's rate **whichever terminal it lands in** — the dilution comes from being counted as a verdict opportunity at all. A new terminal would have appeared to address the metric while leaving the real defect in place. It would also require changes to `terminal`, `queue_map`, the done-fold, the active→done mover, `retro-debt`'s classifier and both board projections — i.e. `work-items.py`, the sole writer, under `IMP-034`'s documented freeze hazard, with two agents live in the tree. And the distinction it buys is already carried: the `cancelled` event's note names the superseding item. **Registered instead as the real finding — the quality denominator counts non-verdict exits (`cancelled`, `blocked`) as verdicts — for the loop to fix, with the evidence attached.** **NO EXPERIMENT ROW for the edge**: a missing edge is a FIX, and §25a/EXP-063 are explicit that a fix never had a row to earn. Scoring "did we add the edge" cannot come back negative.

**REGISTRY.** `EXP-ROC-016` **DELETED, superseded by its own rewrite** — it fired for real and then licensed a 16-day stop, and its target (`reported`+`queue`) went **57.6% → 58.75%**, slightly worse; successor `EXP-ROC-019` lands in the same change per §25a.6. `EXP-ROC-010` **KILLED at strike 3, on its premise rather than its metric**: the prescribed command `make item-brief PROJECT=<p> ID=<id>` **has never existed** — the target takes `IDS=` and exits 2 on `ID=` — and what it emits is a one-line triage table (id/state/value/cost/defer/title truncated to 100 chars) that structurally cannot be *"the first read in every dispatch brief"*, because it never prints a premise or an acceptance. Two cycles of zero adoption were not orchestrator neglect; **the instruction was unexecutable, and nobody ran it once to find out.** `orchestrator.md` corrected to cite the real interface for its real use — backlog triage, `QUEUE=intake`. ROC 5/8 active, under cap.

**CONSTRAINT: UNCHANGED for the 13th consecutive close** — `reported` / owner `orchestrator`, 35.41% of GLT, median 43,364s (12.05h) across 126 items, 0.00% backfill. But its dominant MECHANISM is now named and it is not slow triage: at 10% uptime the system is switched off for nine days in ten, and a mean of 4.8 days in `reported` against a 12-hour median is what being switched off looks like. **EXPLOIT before elevate: the fix is to stop the loop stopping, not to triage faster.** Subordinate: caps stay where they are — `wip` is not full, it is **occupied**, and raising it is explicitly refused. ROUTED to `.claude/hooks/loop-continue.mjs`, `.claude/tools/loop-uptime.js`, `Makefile`, `process/machinery/state-graphs.json` (v12), `.claude/skills/work-items/scripts/test_work_items.py`, `.claude/tools/linear-project.py`, `process/linear-mapping.md`, `.claude/agents/orchestrator.md`. **`OI-ROC-013` is CLOSED BY THIS CHANGE** — it asked for exactly this and was the oldest aging item in `ready`. -->

<!-- v175 (OWNER RULING, ROC 2026-08-29; NOT a retro. The owner, on seeing DEF-ROC-162: *"this is why the orchestrator is supposed to run the wi-\* commands. you tried to say this was a problem before but this is evidence that it is not a problem and it should not be run from subagents."*

**§F13 IS REVERSED. THE ORCHESTRATOR IS THE SOLE EXECUTOR OF `wi-*` COMMANDS; A SPECIALIST NEVER RUNS ONE.**

**The evidence is `DEF-ROC-162`:** `wi-project` loads a snapshot of all 373 item files and writes them all back — no lock, no re-read, no compare-and-swap — so **any `wi-append` landing during the run is silently and permanently destroyed**. Both commands exit 0. Neither warns. **`wi-validate` reports the store CLEAN afterwards**, because the invariant is `derived == fold(events)` and that holds just as well over a log with an event missing from it — a validator that cannot see the loss. A real `confirmed` event was destroyed and was caught only by the `DEF-ROC-154` re-read-from-HEAD habit. **That race requires two concurrent writers, and §F13 supplied the second.**

**THE LEDGER ON §F13, WHICH WAS MY OWN v163 EXPLOIT.** v163 named the orchestrator's serialisation of state events as the constraint and retired a standing prohibition against specialists running `wi-*`. **v164 — the owner, within the hour — falsified that with one number**: agent work-effort is **0.2% of gross lead time**, so a serialisation inside 0.2% cannot explain a 99.8% wait figure. `EXP-ROC-015` was scoped to "a real but SECONDARY inefficiency" on the spot and **never scored a single positive**. So §F13 bought at most ~0.2% and cost silent permanent loss of the single source of truth. Row **KILLED**, and killed for a reason the registry has not used before: not that the metric failed to move, but that **the change it scored caused a defect**.

**WHAT IS NOT REVERSED, because only one half of §F13 was wrong.** The **v11 rights model** (`OI-ROC-006`) stays exactly as written: rights derive from the item's declared owner and a verdict is the tester's alone. It **refused the orchestrator three times in one session and was right every time**. It governs *whose judgement an event records*; §F13 was about *who types the command*. **§F13a** (turn-ending) is likewise untouched — that was v164's real constraint.

**THE COST OF REVERSING, STATED RATHER THAN HIDDEN:** a transcribing orchestrator can soften or mis-state a verdict it did not reach. Two guards, both enforceable: **it may not fire a verdict the specialist did not explicitly state**, and the event note carries the specialist's own evidence rather than a paraphrase. If a report is ambiguous, ask — do not decide on its behalf.

**`DEF-ROC-162` STAYS OPEN.** Serialising through one actor is an **operational mitigation, not a fix**: a store that can silently lose its own source of truth under *any* race is wrong regardless of who usually writes it, and a second orchestrator — another project's instance, or a human at a terminal — can still race it.

**PRINCIPLE-FAILURE LOGGED** (`2026-08-29-roc-retired-a-protective-constraint-chasing-an-efficiency-worth-0.2pct.md`), and its lesson cost more than the rule: **a constraint that looks like pure overhead may be holding a safety property nobody wrote down.** The prohibition had a *stated* rationale — a measured hazard where an engineer editing `work-items.py` froze every state change for hours — and I checked that rationale, judged it an over-applied proxy, and retired it. **The rationale I checked was not the only thing it was doing.** Its protective effect on the append/project race was written nowhere, so removing it looked free. This is the mirror of the family this project registers most often: usually a control that asserts nothing, here a control that asserted something real nobody had named — both invisible for the same reason, that nothing states what the mechanism is *for*. **Before retiring a constraint, ask what it prevents that is NOT written down, and leave it until that question has an answer.** ROUTED to `process-current.md` §F13, `.claude/agents/orchestrator.md`, `.claude/agents/cicd.md`, `.claude/commands/loop-run.md`. -->

<!-- v174 (OWNER SHARPENING of v173, ROC 2026-08-29, within the hour. The owner: *"we should be doing refactor, red green not red green refactor."*

**v173 GOT THE DIRECTION RIGHT AND THE GRANULARITY WRONG, and the difference is not cosmetic.** v173 wrote §F14 as an ENTRY GATE — one refactor at the start of an item, followed by many `red → green` cycles. The owner's sharpening makes the refactor **the first beat of EVERY cycle**: before each failing test you shape the code to receive it, then write the test, then make it pass, and round again. **§F14 is retitled from *"the engineering step has an ENTRY GATE"* to *"the TDD cycle is REFACTOR → RED → GREEN"***, which is what was actually meant.

Why the granularity matters rather than being a rewording: an item-level entry gate is **one** look, taken before the engineer knows what the code will need — the understanding of where the design resists arrives DURING the work, not before it. Putting the refactor in the cycle means it happens at the moment the resistance is discovered, and it shrinks the blast radius further, because a cycle's change names a smaller surface than an item's does. It also removes the escape hatch v173 accidentally left open: *"I refactored at the start"* would have discharged the obligation for an entire item.

**Kent Beck's formulation is adopted because it is shorter than any of our explanation: *make the change easy, then make the easy change*.** The refactor is what makes it easy, so it necessarily comes first — that sentence carries the whole rule and is now quoted in §F14, `engineer.md` and `delivery-principles`.

ALSO CLARIFIED, because the word was doing two jobs: **if the tree is not green when you arrive you are FIXING, not refactoring.** Different act, different name. Conflating them is how "refactor" becomes a word for any edit made while the build is broken, and it is why the cycle's first beat specifies a green start rather than assuming one.

ROUTED to the same four places v173 touched — `process-current.md` §F14 (retitled, rule restated as the three-beat cycle), `.claude/agents/engineer.md` (steps 2 and 3 merged into one cycle step, renumbered), `.claude/commands/loop-run.md`, `.claude/skills/delivery-principles/SKILL.md` — because a rule stated at the wrong granularity in four places is wrong in four places. **No new experiment row: `EXP-ROC-018` already scores this change and its arms are unaffected** — the negative arms (lead time rises while neither `casesWithNoVariation` nor the coupling ratchet moves; "I looked and left it" on every item; refactor and feature in one commit) all read the same at cycle granularity, and §25a is explicit that a correction to a change already under measurement is not a second experiment. -->
...
**The non-obvious mechanism, and it is now behind three separate defects here** (`DEF-ROC-086` dark deploy, `DEF-ROC-143` items-only commit, this): **a SKIPPED job is not a failed job, AND it still satisfies a dependent's `needs:`.** So a gate can be disconnected without anything going red, by an edit that looks like configuration rather than deletion. Folded into §F11.4 as clause 5 rather than opened as a new section — it is the same rule's third layer (does the gate SPEAK / can it SKIP / is it CONNECTED), and §36 warns against accretion.

**Placement is load-bearing and worth copying:** the guard lives in `src/app`, not in the dashboard suite, **because a guard in the same suite would be deleted by the same edit that deletes what it guards.** Both jobs sit in the closure, so the guard reds from a different job than the one being protected.
...
**5. IS THE GATE CONNECTED? A skipped job satisfies a dependent's `needs:` [v170].** The
third layer of this rule — clause 1 asks whether the gate SPOKE, §F11.5 asks whether it could
silently SKIP, and this asks whether it is still WIRED to the thing it gates. `DEF-ROC-146`:
no test read the workflow, so the gating step could be deleted and every offline check, both
lints and every other CI job would stay green. It had been deleted **twice**.

**A skipped job is not a failed job, and it still satisfies `needs:`.** That single fact sits
behind three separate defects here (`DEF-ROC-086`, `DEF-ROC-143`, `DEF-ROC-146`), so a gate can
be disconnected by an edit that reads as configuration rather than deletion. Where a gate's
verdict is load-bearing, **assert its wiring**: that the step exists, carries no
`continue-on-error` and no `if:`, that its job carries no `if:`, that the job is inside the
gated `needs:` closure, and that the trigger's path filter admits every input the gate reads —
which is not the same set as the code under test.

**Put that guard in a DIFFERENT suite from the thing it guards**, inside the same closure.
A guard in the same suite is deleted by the same edit that deletes what it protects.
...
## F14. The TDD cycle is REFACTOR → RED → GREEN [v173, sharpened v174, ROC]

**Owner ruling, 2026-08-29:** *"ensure the process has the engineer doing refactors before
starting work… this is incorrect. We should refactor before something new, ensure tests pass
and then start the new thing."* **Sharpened within the hour, and the sharpening matters:**
*"we should be doing refactor, red green not red green refactor."*

**So this is not a once-per-item entry gate — it is the SHAPE OF EVERY CYCLE.** v173 wrote it
as a step that runs at the start of an item, then many `red → green` cycles after it. That was
too weak. The refactor is the **first beat of each cycle**: before every failing test you
shape the code to receive it, then write the test, then make it pass. Repeat.

It was previously stated last, in four places: §F11's exit gate (*"the work you are about to
HAND OVER"*), `engineer.md`, `delivery-principles`, and the `loop-run` dispatch line. §F11
keeps the **check** (did it get worse); §F14 owns the **improvement**.

Kent Beck's formulation says the same thing and is worth borrowing because it is shorter than
any of this: **"make the change easy, then make the easy change"** — the refactor is what makes
it easy, so it necessarily comes first.

### The rule — the cycle, every time

**REFACTOR → RED → GREEN.**

1. **REFACTOR.** On a green tree, shape the code to receive the change you are about to make.
   Behaviour-preserving; the tests are unchanged in what they assert. Re-run them and prove
   still green — a refactor you do not re-verify is a change, not an improvement. **Commit it
   separately**, before a line of the new behaviour exists.
2. **RED.** Write the failing test for the next increment.
3. **GREEN.** The minimum code to pass it.

Then round again. **Every cycle opens with the refactor**, not just the first one of an item.

**If the tree is not green when you arrive, you are not refactoring — you are FIXING.** That
is a different act with a different name, and conflating them is how "refactor" becomes a word
for any edit made while the build is broken.

**The separate commit is not bookkeeping.** A refactor mixed into a feature diff is
unreviewable and unrevertable — nobody can tell which hunk moved code and which changed
behaviour, so the whole thing is trusted or reverted as one.
...
### Why the refactor opens the cycle rather than closing it
...
* **Before is scoped by construction.** The coming change names the blast radius. "Refactor
  what you are about to touch" is bounded; "improve the codebase" is not — and at cycle
  granularity the radius is smaller still, which is the point of putting it in the cycle
  rather than at the item boundary.
