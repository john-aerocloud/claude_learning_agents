...
- **No `git push`.** Agents commit to **LOCAL trunk only** — never `git push`. A
  `push` is an **outward-facing, hard-to-reverse action** to a destination the
  agent cannot verify; work repos are local-by-default and many have **no remote**
  at all. Giving a repo a remote and pushing is a **human-gated** decision (the
  human owns where code is published). An agent that thinks a push is needed
  reports it and stops — it does not push.
