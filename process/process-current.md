...
supersedes: v121, v120, v119, v118, v117, v116, v115, v114, v113, v112, v111, v110, v109, v108, v107, v106, v105, v104, v103, v102, v101, v100, v99, v98, v97, v96, v95, v94, v93, v92, v91, v90, v89, v88, v87, v86, v85, v84, v83, v82, v81, v80, v76
status: active
---

<!-- v122 (FOCUSED incident retro, AdixOut 2026-07-30; §F8 retro-debt INCIDENT gate — DEF-AIDX-010 defect-resolve + SLC-013/SLC-014/CHK-012 closes; second retro of the session, on top of v121 which covered the first-prod-deploy story). CONTEXT — the whole OAG dev+prod cutover DELIVERED this session: first-ever prod stack live and admitting the REAL OAG feed (~1.8–2.8k deliveries/hr, OAG-confirmed), dev migrated to the dedicated AidxOut-dev-IngestBus path, legacy oag-consumer-bus co-tenancy retired (UC-037) — SLC-013/SLC-014/CHK-012 done. REQ-004 remains `blocked` (honest): its cutover scope is complete but two newly-found, externally-gated feed-inherited defects hang off it (DEF-AIDX-011 no OagFlightCancelled ever emitted / OAG-194; DEF-AIDX-012 ~78% of legs lack scheduled departure / OAG-195 — both blocked on OAG shipping fixes + backfill). ONE lesson, TIGHT (single routed change, no experiment row — a fix/discipline-tightening is not an experiment, §25a): DEF-AIDX-010 was marked CONFIRMED and a prod IAM change shipped on the strength of an external report ("100% NO_PERMISSIONS") + a plausible structural diff (prod grant had an extra aws:PrincipalAccount condition dev lacked), WITHOUT a live reproduction of the CURRENT symptom — which would have shown delivery already working (2171 rule invocations/45min, 0 failures, under the OLD grant; read model growing with real legs). The report was STALE (pre-bus-go-live window). The shipped grant-simplification was harmless (aligned prod with dev) but unnecessary, and I flip-flopped the story to the user twice, even 'correcting' a CORRECT prior live read to match the stale report. ROUTED CHANGE → .claude/commands/defect.md (reproduce-to-confirm gate): CONFIRM now REQUIRES a live reproduction of the symptom against CURRENT state — a trusted/relayed report + a plausible root-cause is NOT confirmation; assert the current live signal first; if it does not reproduce now, record unconfirmed/not_reproduced (stale report) and STOP. assert-real-state (EXP-115) applies to the DIAGNOSIS, before the fix, not only to post-fix validation. Targets CFR + wasted-cycle/lead-time (a stale-report 'defect' no longer authorises a change). Founding: principle-failure 2026-07-30-adixout-def010-diagnosed-from-stale-report-without-live-assert.md. CONSTRAINT unchanged (registered/queue dependency artifact — not in-system squeezable; budget not spent there; this is a justified safety/CFR discipline fix under the constraint-gate). Registry unchanged: 8 active (AT cap-8; EXP-101 adopt-eligible next cycle). No global-section rule changed beyond the defect-gate tightening; a human-authorized prod bus-policy change + a security-guard exception this session are recorded in work/AdixOut/decision-log.md, not here (project-scoped). -->
