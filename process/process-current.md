...
<!-- v89-retro (OAG 2026-07-22, SLC-041 cross-account-push RESET; retro-debt gate tripped by 4 incidents: UC-XA8/UC-XA10 rejects + DEF-XA1/DEF-XA2 resolves). NO version bump — both learnings routed to narrowest-owner agent files (v87 precedent; also process-v90 tag already exists on main from a concurrent instance). DORA (all-time / trailing-30d): deploy-freq 9.38/9.17 per active-day (healthy); lead-time median 725s/28110s, p85 237048s/332400s (p85 inflated by the 07-16→07-21 EXTERNAL dev-shared IAM-access block, not system rework); CFR 5.1% (6 deploy_failed+rejections); MTTR median 15481s, mean 169200s (mean inflated by the same external block — the 437069s "validation-rejection" MTTR is human/access latency, not recovery-engineering). CONSTRAINT: top GLT owner = `queue` 39% (registered 21% + ready 18%), contaminated by the reset re-derivation + external block; the ACTIONABLE constraint this cycle was the DEPLOY-VERIFICATION LOOP — UC-XA10 took 5 deploy attempts. HONEST CFR/MTTR read: the churn was DEPLOY-MACHINERY, not product/topology — the CX-5 topology premise was CORRECT; the CX-3 "primary" (direct cross-account Pipe target) was a genuine architecture-premise failure, but it took 5 attempts to find ONLY because (i) a FALSE-GREEN infra deploy (UpdatePipe swallowed the target change → split-brain AccessDenied) hid it, and (ii) it was built as "primary" on doc confidence rather than probed first. WHY-CHAIN: 5 attempts ← each failed on a different deploy-mechanism issue found only at deploy time ← (a) provider "Updated" trusted as proof-of-apply + (b) an unverified external capability built-upon ← deploy-success was trusted as a proxy for BOTH resource-applied AND mechanism-possible, neither independently verified. ROUTED: EXP-111 → cicd.md (a deploy is not green until the LIVE resource is read back and asserted == declared intent; force real replace where Update* carries a CREATE-only field) [CFR]; EXP-112 → solution-architect.md (probe a load-bearing UNVERIFIED external CAPABILITY with the cheapest live probe BEFORE building the primary on it; docs ≠ proof) [lead time + CFR]. SCORED PRIOR: v88/EXP-109 (deploys pipeline-only; dev=integrated pre-prod gate) NET WIN — the whole reset shipped through CI (runs 29847120648→29936426797), every CFR hit dev-contained, prod deploy clean first time; v89/EXP-110 (multi-audience DoD) FIRING for the support-runbook leg (2 runbooks captured error pathways as-found: pipe-target-silently-swallowed, cross-account-pipe-target-forbidden — both confirmed accurate), marketing/delivery-tree legs too-early-to-score; EXP-108 (deploy_failed→CFR integrity) VALIDATED/adopt (fired 3× UC-XA10, CFR 0→5.1%). Runbooks (c) confirmed correct. Token review: dominant spend = the 5-attempt deploy-verification loop on UC-XA10; both routed changes ARE the token lever (probe-first + post-deploy assertion cut the disprove-by-deploy cycles). -->
