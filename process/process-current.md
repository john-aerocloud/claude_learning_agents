...
- **Orchestrator** reads the dependency edges as the parallelism plan: use
  cases with no mutual dependency MAY be built by parallel engineers (worktree
  isolation, sequentially independent WIP). Shared-file mutations stay
  sequential (same rule as route independence notes).
