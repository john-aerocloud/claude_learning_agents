...
difficulty. They differed in whether a command returned non-zero.
...
2. **Ready below floor** — `len(ready) < min_items` (§F2).
3. **Queue over cap** — any queue depth > its `wip_limit` (§F2).
