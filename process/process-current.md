...
> **v55 (SLC-003 retro, 2026-06-17 — OagEventSource).** Target metric: **gross
> lead time** (the per-node-command PATH tax) + **flow observability**. Evidence:
> EXP-050's mid-session `settings.json` `env` edit did NOT reach a fresh subagent
> shell (UC-16 plain `npm` → command-not-found) — same session-start config-
> resolution class as the EXP-039 `model:` finding. v55 generalises §16 with the
> **session-start config-resolution rule** (config that the harness reads at
> session start cannot rescue the running session — bridge mid-session via a
> mechanism that does not depend on the inherited env), and routes the durable
> fix to a committed Make/wrapper (cicd). EXP-052 (idempotency-extension cost)
> graduated to a product cheap-UC class on n=2; EXP-051's flow-manager reasoning
> validated but its METRIC residue routed to EXP-053. Other changes are
> integrations/prunes, scored in `process-history/v54-2026-06-17.md`.
...
## 3. CFR ledger convention (definitional — v51, EXP-044)
...
**Scoring quarantine (v51, EXP-046):** a model-tier change is a confound for
every DORA-scored experiment, because a metric move during the change could be
the model, not the change. When any agent's `model:` changes, the retro **opens
a quarantine window** (note it on the experiment registry header with the date +
which agents moved). Experiments scored inside the window MUST flag
`model-confounded` in their scoring note and **may not be marked `validated` on a
DORA move alone** — they need either a mechanism-level confirmation (the
behaviour demonstrably fired, independent of the metric) or a scoring opportunity
after the window closes. Where feasible, hold one comparable agent on the prior
tier as a control. The window closes when the next retro judges the tier stable
(default: 2 slices on the new tier with no further model change).
...
- **Commit TARGET — two separate repositories (v50).** Each `work/<project>/`
  ...
  **Agent-structure and process changes** (`.claude/`, `process/`, `CLAUDE.md`,
  ...
  (cf. the bare-root-`slices/` principle failure). [EXP-043]
- **No `git push` (v53, EXP-049).** Agents commit to **LOCAL trunk only** —
  never `git push`. A `push` is an **outward-facing, hard-to-reverse action** to
  a destination the agent cannot verify; work repos are local-by-default (v50)
  and many have **no remote** at all. Giving a repo a remote and pushing is a
  **human-gated** decision (the human owns where code is published). An agent that
  thinks a push is needed reports it and stops — it does not push. (Triggered by
  the solution-architect attempting to push the OagEventSource repo, 2026-06-17 —
  no-op as there was no remote, but an unverified-destination attempt all the same.)
