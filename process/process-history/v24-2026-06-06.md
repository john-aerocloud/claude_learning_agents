...
# Retro scoring at retirement (s005 delivery retro, 2026-06-06)

DORA at close: lead(median)=3618s, freq=5/active-day, cfr=27% (prod-reaching
only, §31), mttr=3054s (s005 pair: 5807s), constraint=tester.

| Change | Anticipated | Observed | Verdict |
|---|---|---|---|
| v14 §28 auto-retro | zero human-prompt wait delivered→retro | retro fired immediately at s005 delivery; +1 human-invoked focused retro mid-slice | MET |
| v14 §29 parallel documenter | docs off critical path | overlapped both at s004 retro and s005 (runbook in parallel with this retro) | MET |
| v14 §30 cross-stack contract | CFR 0% for the CDN↔route class | class did not recur (D1/T7 pinned); CFR cause moved to NEW classes (platform semantics, code↔policy, browser config) | MET for its class; extended this retro |
| v14 §31 CFR convention | metric integrity | 4 pipeline_failures excluded; CFR 27% = prod-reaching only | MET |
| v14 §4e same-session recovery | MTTR <600s, same-session | same-session YES (vs s004's 9h gap); 5807s >600s — defect had 6 stacked causes | PARTIAL |
| v15 §32 improvement slices | scheduled, measured improvements | IMP-001/002/003 specced→delivered→scored | MET |
| v15 §33 / IMP-001 zero prompts | zero prompts/slice | near-zero: agents report none avoidable; residual = novel surfaces (closed in-slice) + one sandbox override | NEAR-MET |
| v15 §34 chunk plan | slices add up visibly | maintained at slice-next + delivery | MET |
| v16 §35 / IMP-002 validation-as-code | tester prompt-free, run provenance | all validation through framework; validation_run rows with sha; caught real GSI-ARN drift; relevancy review done | MET |
| v17 §36 routing (+v24 correction) | narrowest-owner placement | ~10 routings; framing corrected to optimal-not-minimal | MET |
| v17 / IMP-003 parameterised ops | no inline assembly | make targets used throughout; tester EXTENDED them (v23 self-service) | MET |
| v18 §37 use cases | parallel build wall-clock < serial sum | Set A (1280s) ∥ Set B (539s): saved ~539s ≈ 30% of build phase; zero collisions | MET |
| v19 §38 open-items register | nothing evaporates | 27 OI items; all flagged residue tracked; selection logged | MET |
| v20 §39 scheduling over compensation | zero compensation fixes | premature var undone; capture-then-set honoured at E3; R2 drain documented as genuine-runtime | MET |
| v21 §40 UC flags | shared-seam parallelism | NOT exercised (file-ownership sufficed; Set C single-engineer) — but 2 stash incidents prove the need | UNSCORED — carry |
| v22 §41 hexagonal/supportability | attribution from logs | partially shipped early: R2 added categorised logging to ws handlers (used in diagnosis same day!); OI-17/18 carry | EARLY SIGNAL POSITIVE |
| v23 tooling self-service | no orchestrator tooling round-trips | tester self-built env forwarding; engineer fixed ws-probe; zero round-trips | MET |
| principles/01 version identity | distribution vs behaviour separable | evidenced twice (smoke race; runbook gap); implementation OI-25 pending | PENDING IMPL |
