...
# RETIREMENT NOTE — superseded by v26 (refactor-process)

v26 is a placement/structure refactor of v25 (changes WHERE rules live, never
WHAT they require). v25 had accreted across 12 same-day versions (v14–v25) in
invention order with scored v-annotations, project-specific references, a
duplicated baseline in three places, and per-slice historical tables inline.
v26 restructures around process STAGES, removes the misplaced/redundant, and
keeps one current baseline. Full v14–v24 anticipated-vs-observed scoring is in
`v24-2026-06-06.md` and was not duplicated forward.

## Full routing table (v25 rule → v26 home)

| v25 § / content | Class | v26 destination |
|---|---|---|
| §1 Operating principles | GENERAL | v26 stays (Principles & metrics) |
| §2 Gross-lead-time DEFINITION | GENERAL | v26 stays (Principles & metrics) |
| §2 per-slice wall-clock table + constraint-class narrative | SCORED-HISTORICAL | DROPPED from current (lives in v14–v24 history); compressed to current constraint truth |
| §3 Wait-time taxonomy (model) | GENERAL | v26 stays (Principles & metrics); oxo examples generalised |
| §4 Session continuity | GENERAL | v26 stays (Build/validate); s004 narrative dropped |
| §5 Time-to-first-deploy metric+target | GENERAL | v26 stays; project table DROPPED (history) |
| §5 project table | SCORED-HISTORICAL | DROPPED from current |
| §6 Delivery gap | GENERAL | v26 stays |
| §7 Loops | GENERAL | v26 stays |
| §8 Gate design | GENERAL | v26 stays (Selection/gates); GATE-4-S004 ref generalised |
| §9 Project classification | AGENT-SPECIFIC (architect) | `.claude/agents/solution-architect.md` "Project classification"; 1-line pointer in v26 |
| §10 Agent roster | GENERAL | v26 stays (Selection/gates) |
| §11 Tester scope | AGENT-SPECIFIC (tester) | already in `tester.md`; v26 keeps 1-line pointer; historical median DROPPED |
| §12 Deploy event definition | GENERAL | v26 stays (Deploy) |
| §13 Deploy event logging | GENERAL (orchestrator duty) | v26 stays (Deploy) |
| §14 Engineer duration_s | AGENT-SPECIFIC (engineer) | `engineer.md` DORA duty; removed from v26 |
| §15 per-project.md discipline | GENERAL (orchestrator) | v26 stays (Retro) |
| §16 DORA baseline v14 table | SCORED-HISTORICAL | replaced by ONE current baseline block in v26 |
| §17 Commit discipline | GENERAL | v26 stays (Build) |
| §18 Two-pipeline structure | AGENT-SPECIFIC (cicd) | `cicd.md` "Two-pipeline structure"; removed from v26 |
| §19 CICD pre-flight checklist | AGENT-SPECIFIC (cicd) | `cicd.md` "Pipeline pre-flight checklist"; removed from v26 |
| §20 Pipeline fail-fast YAML | AGENT-SPECIFIC (cicd) | `cicd.md` "Pipeline fail-fast config validation"; removed from v26 |
| §21 Architecture-lite no-backend path | AGENT-SPECIFIC (architect) | `solution-architect.md` "Architecture-lite path"; removed from v26 |
| §22 Surface-change done condition | AGENT-SPECIFIC (engineer) | `engineer.md` "Smoke-test discipline"; removed from v26 |
| §23 Stable smoke selectors | AGENT-SPECIFIC (engineer+tester) | `engineer.md` + `tester.md`; oxo selector list → `/work`; removed from v26 |
| §25 Working-directory convention | GENERAL (command-form, all agents) | v26 stays (Build); hardcoded abs path + s004 scored note DROPPED |
| §26 Committed allowlist | GENERAL | v26 stays (Build); scored note DROPPED |
| §27 Change-set queue | GENERAL (OBLIGATIONS — must survive) | v26 stays (Retro & improvement); v-annotations cleaned, unscored items + IMP queue preserved |
| §28 Auto-retro at delivery | GENERAL | v26 stays (Retro); scored note trimmed |
| §29 Documenter runs in parallel | GENERAL | v26 stays (Document); scored note trimmed |
| §30 Cross-stack contract / walking-skeleton / code↔policy — OPERATIONAL detail | AGENT-SPECIFIC (engineer+architect) | `engineer.md` (synth contract + probe + code-policy pin) + `solution-architect.md` (new-mechanism flag); v26 keeps the cross-cutting principle + pointer; oxo evidence DROPPED |
| §31 CFR ledger convention | GENERAL (definitional) | v26 stays (Principles & metrics) |

Sections §32–§41 (improvement slices, tools-over-permissions, chunk plan,
validation-as-code, improvement routing, use cases, open-items register,
scheduling-over-compensation, UC flags, hexagonal/supportability) were already
routed into the agent definitions across v15–v24; v25 carried only dangling
references to them. v26 keeps §34 chunk-plan ownership in `product.md` (added
this refactor) and the §40/§41/etc. behaviours in their agent homes, with
cross-agent pointers where another agent must know the rule exists.
