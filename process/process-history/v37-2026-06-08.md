...
supersedes: v36
status: superseded by v38 (2026-06-08, two prod defects → multi-party modeling + visual-correctness)
---

# Retirement note (scored at v38 retro)
Two latent prod defects found by HUMAN USE of the live game exposed gaps v37
didn't cover: (1) board renders as a line not a 3x3 grid — `.board` has no
`display:grid`, latent since s002, every test passed because geometry is never
asserted (functional-green != visually-correct); (2) copy-control copies the
URL when a user expects the code — the multi-party share flow was modeled from
the sharer's side only. v38 adds §12b multi-party/multi-instance modeling +
routes visual-structural correctness to ui-designer/tester.
