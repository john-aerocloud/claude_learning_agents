...
2. **validated** — anticipated effect observed at retro scoring. The change is
   ...
   slice adds real new craft — that is not an integration failure. After integration the row drops out of active tracking (spot
   re-check only if its metric later regresses). The integration policy is
