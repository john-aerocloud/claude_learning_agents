...
up; fail → Rework. **Replenishment fires when `depth(Ready) < ready.min_items`
— BEFORE starving:** product, in order, (a) decomposes more use-cases from the
current slice; (b) takes the next slice from the chunk (unattended — no slice
gate); (c) advances to the next chunk; (d) if the requirement is done, the loop
reports *starved + requirement complete* and the orchestrator asks the human for
more work. Product estimates value+cost on every item it creates. Batch small
(penny game): replenish more often, less each time. Target: gross lead time,
deployment frequency. [EXP-020]
...
## Anticipated-vs-observed (scored at the v41 retro, 2026-06-09)

**v40 introduced the pull system (EXP-020…EXP-029).** First real exercise: the
`observatory` intake (REQ-OBSERVATORY + CHK-1..7 registered, costed, CHK-1
enqueued, capabilities scoped).

- **What held:** the work-item model (EXP-021) registered 8 items with
  parent/children + value/cost and rebuilt the tree cleanly; the uniform queue
  model (EXP-022) surfaced the Intake below-floor signal correctly; the two-gate
  model (EXP-023) held the one intake gate; the flow-manager separation (EXP-024)
  kept queue mechanics owned and traceable.
- **Gap exposed (→ fixed in v41):** §F3 says the loop "runs continuously" but
  models **replenishment as a serial pre-pull step** and provides **no wake
  trigger**. When the loop has not been started (or has drained/exited) and a new
  item is enqueued onto an empty queue, nothing restarts it. The orchestrator,
  lacking an explicit rule, **asked the human to choose between starting the loop
  and replenishing CHK-2** — two independent parallel processes presented as an
  exclusive human decision. Human-directed correction: both must run
  automatically and in parallel; an enqueue-to-empty must wake the loop.
- **Verdict:** v40 pull mechanism VALIDATED for intake/registration/costing;
  REFINED by v41 §F9 (continuous background operation + parallel replenishment +
  autonomous wake). EXP-020 carried (still 0 delivered slices under the loop).
