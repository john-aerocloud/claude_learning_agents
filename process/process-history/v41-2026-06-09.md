...
v40). Target: gross lead time + throughput. [EXP-020, EXP-022]
...
up; fail → Rework. **Replenishment is a PARALLEL, independent process — not a
serial pre-pull step (§F9).** It fires the moment `depth(Ready or Intake) <
min_items` and runs **concurrently** with the build of already-pulled UCs: while
engineers build the current set, product, in order, (a) decomposes more
use-cases from the current slice; (b) takes the next slice from the chunk
(unattended — no slice gate); (c) advances to the next chunk; (d) if the
requirement is done, the loop reports *starved + requirement complete* and the
orchestrator asks the human for more work. Lifting a below-floor queue above
floor and pulling ready work never block each other. Product estimates
value+cost on every item it creates. Batch small (penny game): replenish more
often, less each time. Target: gross lead time, deployment frequency.
[EXP-020, EXP-030]
...
real gates are untouched). [EXP-030]

---

## Anticipated-vs-observed (scored at the v42 retro, 2026-06-09)

**v41 introduced §F9 (continuous loop + parallel replenishment + autonomous wake, EXP-030).** First exercise: the s001-read-layer slice (6 UCs delivered on trunk, 92/92 green).

- **VALIDATED strongly:** during the build, the orchestrator asked the human ZERO flow-mechanics questions; `loop_wake` fired and the loop ran unprompted; replenishment (product decomposing CHK-1) overlapped the build; the parallel wave (UC2/3/4) ran at **parallelism efficiency 1.00 with 0 collisions** — the §39 scheduling caught both hidden seams (scaffold, server.js mount) PRE-build at zero rework. EXP-026/EXP-027 had a clean first win.
- **Gap exposed (→ fixed in v42):** §F9 stopped the orchestrator asking *mid-loop*, but at the **slice boundary** the orchestrator still ended its turn to ask the human "close the slice + retro, or pause to inspect?" — a stop the user rejected ("stopping like this adds a lot of gross lead time — keep on trucking"). §F9 named "start the loop / replenish-or-pull" but not "advance through slice-done → retro → next chunk." Second data point of the orchestrator-inserts-an-avoidable-stop class.
- **Verdict:** v41 §F9 net win on lead time (parallel build, no mid-loop idle); REFINED by v42 (keep-trucking: slice/chunk/retro boundaries are autonomous, not human checkpoints). EXP-030 carried 1/2.
