...
supersedes: v53
status: superseded-by-v55
---

## v54 change-set — anticipated vs observed (scored at the SLC-003 retro, 2026-06-17)

v54 routed three changes off the SLC-002 retro. Scored against SLC-003 evidence
(UC-12..16, the offline-normaliser chunk-family close):

- **EXP-051 (shared-source-file seam edge → §F6 + flow-manager.md).** Anticipated:
  flow-manager stops reporting phantom max>1 on shared-file pulls; par_eff becomes
  a true signal. **Observed: PASS at the flow-manager layer (2/2).** Both SLC-003
  pulls (PULL-C2 UC-12..15, PULL-C3 UC-16) were correctly annotated as serial
  on the shared `normaliser-core.ts` seam under §40, with the schedule recorded as
  the achievable set, not a missed opportunity. **Residue: the `dora.py flow`
  METRIC still divides raw `achieved/max` (0.25) without reading the seam reason,
  so flow.md prints a phantom par_eff even though the flow-manager's reasoning is
  correct.** Routed forward in v55 (the metric must honour the seam annotation).
- **EXP-052 (idempotency-extension cost note → product.md).** Anticipated: product
  cost tracks observed near-zero build cost. **Observed: 2nd data point (n=2).**
  UC-16, like UC-11, required ZERO production code. Decision recorded at v55.
- **EXP-050 (node-on-PATH `env` block → settings.json).** Anticipated: 0 inline
  PATH-prefix invocations; plain `npm --prefix` resolves first try. **Observed:
  FAILED THIS SESSION** — UC-16's fresh subagent shell still hit `command not
  found: npm` on plain `npm`, falling back to the inline prefix. Diagnosis +
  re-route at v55 (EXP-050 → failed-this-session, not succeeded).
