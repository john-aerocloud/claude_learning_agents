...
supersedes: v55
status: superseded-by-v57
snapshot_taken: 2026-06-18 (SLC-004 retro)
---

# Process snapshot — v56 (token-efficiency economic ledger)

> **SNAPSHOT NOTE.** This captures the process AS IT STOOD when v56's changes had
> landed in the BODY (§24 token = cost side of the economic ledger; §25a.6
> `failed` rows DELETED not archived; §26 standing token-efficiency review;
> EXP-054 static-PATH; EXP-055 token review) but the header had not yet been
> bumped from its v55 stamp. The SLC-004 retro (2026-06-18) snapshots it here as
> the true v56 and writes v57 as `process-current.md`.

## Anticipated-vs-observed — v56 change-set (scored at the SLC-004 retro)

| v56 change | target metric | anticipated | OBSERVED (SLC-004 cycle) | verdict |
|---|---|---|---|---|
| §24 — token = COST side of the economic ledger; optimise DORA-per-token | token cost/run vs all DORA | cuts/increases scored honestly, no DORA traded for tokens | frame held: this retro ran the step-4a review, found the cicd-subagent share dominant, chose a DORA-protecting reduction (committed deploy pre-flight, not a re-spawned fat cicd agent) | HOLDS (EXP-055 1/2) |
| §25a.6 — `failed` rows DELETED outright, no archive line | registry simplicity | failed rows stop polluting the working registry | EXP-050 DELETED, thread carried by successor EXP-054; registry stayed lean | HOLDS |
| §26 — standing per-retro token-efficiency review (dogfood step 4a) | token cost/run + orchestrator overhead | each retro records a token estimate + lands one scored optimisation | done this retro (estimate + one optimisation; first dogfood pass) | HOLDS (EXP-055 1/2) |
| EXP-054 — static absolute PATH in settings.json `env` (no `${PATH}`) | gross lead time (PATH tax) + shell integrity | plain node/npm/git/cat resolve first try in a FRESH session | this retro session STILL needed the inline `export PATH=…` (human-supplied) — static value is committed but a fresh validating session has not run (§16.5 session-start resolution) | CARRY (0/1, leg-b Make-wrapper still open) |
| settings.json self-mod allow-rules (settings.json, agents/**, process/**) | gross lead time (0 prompts on retro self-edits) | retro edits its own files with no prompt | edited process/experiments/history with 0 prompts — rules fired | HOLDS |
| CI workflows relocated parent→project; OIDC slug → AeroCloudSystems/Spike-FlightEventSource-OAG | data-integrity (EXP-043 boundary) | CI assets live in the project repo | 0 work/OagEventSource paths in parent commits (EXP-043 grep clean) | HOLDS (feeds EXP-043) |

---

# Current Process — v55 (body as captured; header was stale — true v56)
