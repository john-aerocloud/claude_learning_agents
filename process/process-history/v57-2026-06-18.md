...
supersedes: v56
status: superseded-by-v58
snapshot_taken: 2026-06-18 (delta-005/006/007 mega-session retro)
---

# Process snapshot — v57

> **SNAPSHOT.** Captures the process as it stood under v57 (SLC-004 first-infra
> retro). The mega-session retro (CDK→SST migration + delta-005 ingest +
> delta-007 Fargate + delta-006 read model) snapshots it here and writes v58 as
> `process-current.md`.

## Anticipated-vs-observed — v57 change-set (scored this retro)

| v57 change | target metric | anticipated | OBSERVED (mega-session) | verdict |
|---|---|---|---|---|
| §17.1 / EXP-056 — assert external/cross-account resource ids before first deploy; committed deploy pre-flight | CFR (first-deploy rollback) + MTTR + token | a cross-account/external dep cannot reach a real deploy unconfirmed; rollback→re-spawn loop replaced by a cheap pre-flight failure | OTel layer ARN was the founding case; this session's deploys (SST sandbox, ingest, Fargate, read-model UC-A3) hit NO external-id rollback — UC-A3 deployed in ONE attempt. But the cycle's NEW prod failures were a DIFFERENT class (IAM read-then-write, ESM bundle) §17.1 does not cover | HOLDS for external-ids (EXP-056 1/2); does not generalise → v58 adds EXP-060/061 |
| §11a.1 / EXP-057 — infra-flag defers an unconfirmed external dep, ships the skeleton | deploy frequency + lead time guarded by CFR | a slice with one unconfirmed external dep still ships its core | `otelEnabled=false` shipped the skeleton (SLC-004); this session's migration+Fargate cutover shipped without needing a new infra-flag deferral | HOLDS (EXP-057 1/2, no new opportunity) |
| EXP-058 — architect states per-infra + per-connection fitness functions + default thresholds | observability coverage + MTTR | no infra piece/connection ships without enumerated fitness functions | delta-006 + delta-007 BOTH got Observability & fitness-functions sections (96510ac); UC-A3 shipped with 6 CW alarms; Fargate got checkpoint-lag/DLQ-depth alarms | HOLDS (EXP-058 1/2) |
| EXP-059 — documenter maintains consumer-integration docs AS A SKILL | consumer-onboarding + contract-honesty | a consumable-surface project ships a current consumer SKILL | docs/consumers/SKILL.md written + verified against the live API (documenter 14:08Z) | HOLDS (EXP-059 1/2) |
| EXP-054 — static absolute PATH in settings.json `env` | gross lead time (PATH tax) | plain node/npm resolve first try in a FRESH session | this retro session STILL needed the human-supplied inline `export PATH=…` (pre-dates a fresh reload); static value remains committed | CARRY (0/1, §16.5 — scores next fresh session) |
| EXP-055 — standing per-retro token-efficiency review | token/run vs DORA | each retro records a token estimate + one scored optimisation | done this retro (§4 token estimate); chosen optimisation = build-time bundling+IAM pins that remove the most expensive class (failed prod deploy → fat-cicd remediation re-spawn) | HOLDS (EXP-055 2/2 → VALIDATED) |

---

# Current Process — v57

> **v57 (SLC-004 retro, 2026-06-18 — OagEventSource, FIRST infra-bearing deploy).**
> Target metric: **CFR** (deploy_failure on a first real `cdk deploy`) guarded by
> **MTTR**. Evidence: DEFECT-OAG-001 — the first sandbox deploy of OagFeedStack
> ROLLED BACK on `lambda:GetLayerVersion` 403 against non-existent `dash0-*` layer
> ARNs in the OTel-community account; the delta-001 "CONFIRM at build: exact layer
> version suffixes" item was written but **never actually confirmed** before the
> real deploy. Recovery was clean (999s, deploy_recovery, under the <900s-ish
> target band for a first infra deploy) by **guarding OTel behind an `otelEnabled`
> flag (default false)** so the core walking skeleton shipped and the unconfirmed
> dependency was deferred. v57 routes two cross-agent rules from this:
> **§17.1 — an EXTERNAL / cross-account resource identifier (a Lambda layer ARN,
> a shared resource in another AWS account, a third-party endpoint) must be
> ASSERTED before the first real deploy** (synth-time existence/permission check or
> a `--require-approval`-class probe), never discovered by a `cdk deploy` rollback;
> a written "CONFIRM at build" note is not a confirmation — the confirmation is a
> committed check that FAILS until the identifier resolves. **§40.1 — the §40
> use-case-flag pattern extends to INFRA: an infra capability whose external
> dependency is unconfirmed ships behind a default-OFF flag so the core skeleton
> deploys and the dependency is deferred as an open item** (the `otelEnabled=false`
> pattern). EXP-049 (no-git-push) VALIDATED+integrated on its 3rd confirming point.
> EXP-043 (repo-split) PASS held (0 parent-repo leaks). Token-efficiency review
> (EXP-055 1/2): the cycle's dominant token consumer was the fat cicd subagent
> re-spawns around the deploy round-trip — routed a committed deploy pre-flight to
> cut the failed-deploy re-spawn. v56 change-set scored in
> `process-history/v56-2026-06-18.md`.
...
**As of 2026-06-18 (SLC-004 retro, OagEventSource — FIRST infra deploy):**
cumulative `lead=2543s freq=6/day cfr=21% (15/72 deploys) mttr=843s`;
window(last 12 deploys) `cfr=17% (2/12) lead=1823s` — the window MTTR figure
(32951s) is history-dominated by old observatory overnight rework pairs (§13e),
NOT this cycle: **DEFECT-OAG-001 recovered in 999s** (deploy_failure 06:49Z →
deploy_recovery 07:06Z). CFR rose honestly on the FIRST genuine deploy_failure
(EXP-044 working — CFR can rise on a real deploy break, not just diligence).
**Named constraint: first-deploy external-dependency novelty** — the deploy
queue, not any agent median. The 999s round-trip + the unconfirmed cross-account
layer ARN reaching a real `cdk deploy` is the binding step; the standing attack is
§17.1 (assert external identifiers before first deploy) + §11a.1 (infra-flag
defers unconfirmed deps). Orchestrator (median 900s) and tester (830s) remain the
medians-table constraints for app-only cycles.

**Token estimate (EXP-055, cost side of §24) — SLC-004 cycle.** Dominant
consumers: the **cicd subagent** (multiple 80k–140k-token spawns around the
deploy round-trip — capability step, OIDC pipeline, the failed deploy + the
DEFECT-OAG-001 remediation re-spawn) and orchestration of the infra gate. The
single highest-leverage reduction WITHOUT a DORA cost: a **committed deploy
pre-flight** (synth + external-identifier resolve check, §17.1) so the
failed-deploy → rollback → fresh-fat-cicd-respawn loop is replaced by one cheap
pre-flight failure caught before the expensive `cdk deploy` — removes the most
expensive re-spawn while IMPROVING CFR/MTTR (routed as EXP-056's tool leg). Reject
any cut to the engineer/cicd model tier (would risk CFR on the constraint).
