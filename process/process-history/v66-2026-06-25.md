...
supersedes: v65
status: superseded-by-v67
---

> **v66 anticipated-vs-observed (scored at the v67 retro, 2026-06-25).**
> v66 routed two changes against **CFR + GLT**:
> (1) **EXP-081 — specs assert the invariant, not the incidental** (tester.md +
> engineer.md). *Anticipated:* 0 specs false-fail on an incidental live-data
> condition; data-dependent specs branch on data-state / derive expected from
> per-entity truth. *Observed (SLC-015/023, 1 of 2 scoring opportunities):*
> POSITIVE. UC-FD1 AC-FD1.4 (`framenavigated count <= 1`) was exactly the
> EXP-081 anti-pattern — a raw incidental count, not the invariant. The fix
> recalibrated it to the TRUE invariant (a window sentinel survives = no real
> document reload), and the SLC-023 e2e-fids suite then ran 32-pass/4-skip with
> the BIDS empty-board branch (the v66 conditional) skipping cleanly rather than
> false-failing. Both behaviours fired. Note: the recalibration cost ~1h08m MTTR
> on UC-FD1 — but that was the spec being CAUGHT and corrected at adjudication,
> not a code defect escaping; EXP-081's value is precisely that it reframed a
> would-be "defect" as a spec bug. Stays active (1/2) toward integration.
> (2) **cicd committed `ci-watch` make target.** *Anticipated:* agents stop
> hand-rolling `gh run watch` → GLT. *Observed:* the target now EXISTS
> (`make -C work/OagEventSource ci-watch`, `scripts/ci-watch.sh`) and was used —
> but it MISREPORTED "no runs" because the CI/deploy binding resolved to the
> wrong repo (see v67). The target shipped; its binding is the v67 follow-on.
> Partial: capability landed, correctness gap routed forward.
> Scored POSITIVE alongside: **EXP-080** (verify-at-source) — its 2nd scoring
> opportunity; the SLC-023 deploy status (sha `d45fdce`, CF invalidation id,
> served bundle) was cited verified, and the ci-watch wrong-repo read was caught
> by source-verification, not trusted. EXP-080 now 2/2 → integrate at v67.
