...
| **[C7]** | In-game chat — reuses C4 realtime transport | **closing** — s014 chat send+relay (delta 011) delivered/prod-PASS (cross-instance 199ms). s015 (delta 012, arch-lite) CLOSES C7: NO new infra/route/flow — commits 3 GUARDS over the existing relay: (1) cross-game scope-isolation prod test (a 3rd connection in a DIFFERENT game receives ZERO chat frames — existing by-construction property of the relay, T-CHAT-2 waived from s014); (2) formal p95 <=1000ms proof over >=5 sends; (3) chat input absent on game-over (already-built s014 render gate `result===undefined`, no code change). §9a auto-accept. |
